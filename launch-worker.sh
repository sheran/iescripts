...
if [ ! -f encrypted-wallet.json ] || [ $(cat encrypted-wallet.json | wc -c) -eq 0 ]; then
...
if [ ! -z "${STOPPEDWORKERS}" ]; then
  ...
  if [ ! -z "${RUNNINGWORKERS}" ]; then
    ...
    #cd /home/sheran/code/workerdrop-scripts-public;
    ...
    docker run --hostname $workerName \
             ...
             -v encrypted-wallet.json:/iexec/wallet/wallet_worker.json \
